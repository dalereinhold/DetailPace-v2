...
export function isPending(vehicle: Vehicle): boolean {
  return vehicle.status === "In Progress" && !vehicle.started_at && vehicle.net_work_seconds === 0
}
...
export function formatSecondsToMinutes(seconds?: number | null): string {
  if (seconds == null || Number.isNaN(seconds) || seconds <= 0) return "-- mins"
...
export function elapsedSecondsSince(startedAt: string | null, now: Date = new Date()): number {
