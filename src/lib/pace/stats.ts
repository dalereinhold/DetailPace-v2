...
import { supabase, type Vehicle, type VehicleServiceType, type VehicleType } from "@/lib/supabase"
...
const SERVICE_TYPE_NAMES: VehicleServiceType[] = SERVICE_TYPES.map(s => s.name)
const ESTIMATED_SECONDS_BY_SERVICE: Record<VehicleServiceType, number> = Object.fromEntries(
...
function toTypeStats(group: Vehicle[]): TypeStats {
  ...
  const varianceSeconds = avgSeconds > 0 && estimatedSeconds > 0 ? avgSeconds - estimatedSeconds : 0
  const variancePct =
  ...
  return {
    ...
    estimatedSeconds,
    estimatedFormatted: estimatedSeconds > 0 ? formatDuration(estimatedSeconds) : "--",
    varianceSeconds,
    variancePct,
...
function computeStats(allVehicles: Vehicle[]): Stats {
  ...
  for (const serviceType of SERVICE_TYPE_NAMES) {
    byService[serviceType] = toTypeStats(completed.filter(v => v.service_type === serviceType))
  }

  const byTypeAndService = {} as Record<VehicleType, Record<VehicleServiceType, TypeStats>>
  for (const type of VEHICLE_TYPES) {
    const typeGroup = completed.filter(v => v.type === type)
    const perService = {} as Record<VehicleServiceType, TypeStats>
    for (const serviceType of SERVICE_TYPE_NAMES) {
      perService[serviceType] = toTypeStats(typeGroup.filter(v => v.service_type === serviceType))
    }
  ...
  const completionPct = totalInDb > 0 ? Math.round((totalProcessed / totalInDb) * 100) : 0

  const allCompletedTimes = completed.map(v => v.net_work_seconds).filter(s => s > 0)
  ...
  const overallVariancePct =
    overallEstimatedSeconds > 0 && overallAvgSeconds > 0
      ? Math.round(((overallAvgSeconds - overallEstimatedSeconds) / overallEstimatedSeconds) * 100)
      : 0
  ...
  const typesWithData = VEHICLE_TYPES.filter(t => byType[t].count > 0 && byType[t].avgSeconds > 0)
  let fastestType: VehicleType | null = null
  let slowestType: VehicleType | null = null
  if (typesWithData.length > 0) {
    const sorted = [...typesWithData].sort((a, b) => byType[a].avgSeconds - byType[b].avgSeconds)
  ...
  const overall: OverallStats = {
    ...
    avgFormatted: overallAvgSeconds > 0 ? formatDuration(overallAvgSeconds) : "--",
    estimatedSeconds: overallEstimatedSeconds,
    estimatedFormatted:
      overallEstimatedSeconds > 0 ? formatDuration(overallEstimatedSeconds) : "--",
