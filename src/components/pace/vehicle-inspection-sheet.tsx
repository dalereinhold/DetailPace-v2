...
function VehicleInspectionForm({
  ...
  const [serviceType, setServiceType] = useState<VehicleServiceType>(vehicle.service_type)
  const [status, setStatus] = useState<VehicleStatus>(vehicle.status)
  const [notes, setNotes] = useState(vehicle.notes || "")
  const [netMinutes, setNetMinutes] = useState(Math.round(vehicle.net_work_seconds / 60))
  ...
  const handleSave = async (e?: React.ChangeEvent) => {
    ...
    if (!isValidSwedishPlate(trimmedPlate)) {
      setError("Invalid Swedish plate format (e.g. ABC 123 or ABC 12A).")
      return
    }
              ...
              <div className="border border-border p-4 bg-muted/20 space-y-4">
                ...
                <div>
                  <label
                    ...
                    className="block text-xs font-mono font-semibold uppercase text-muted-foreground mb-1"
                  >
                ...
                <div>
                  <span className="block text-xs font-mono font-semibold uppercase text-muted-foreground mb-1">
                    Classification Type
                  </span>
            ...
            <div className="space-y-4">
              ...
              <div className="border border-border p-4 bg-muted/20 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(["In Progress", "On Break", "Completed"] as VehicleStatus[]).map(st => {
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label
                      htmlFor="net-work-minutes-input"
                      className="text-xs font-mono font-semibold uppercase text-muted-foreground"
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      id="net-work-minutes-input"
                      type="number"
                      min={0}
                      step={1}
                      value={netMinutes}
                      onChange={e => setNetMinutes(Math.max(0, parseInt(e.target.value, 10) || 0))}
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <Button
                        ...
                        variant="outline"
                        size="xs"
                        onClick={() => setNetMinutes(m => Math.max(0, m - 15))}
              ...
              <div className="border border-border p-4 bg-muted/20 space-y-2">
                <div className="flex items-center justify-between border-b border-border/60 pb-2">
                  <label
                    ...
                    className="text-xs font-heading font-semibold uppercase tracking-wider text-foreground cursor-pointer"
                  >
              ...
              <div className="p-3 bg-muted/40 border border-border text-[11px] font-mono text-muted-foreground space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    {copiedId && <span className="text-emerald-500 text-[9px]">Copied!</span>}
                  </button>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1">
          ...
          <div className="border-t border-border pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
              ...
              <Button
                ...
                className="font-mono text-xs gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90 min-w-32.5"
              >
