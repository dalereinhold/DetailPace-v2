import { AlertCircle, ArrowRight, Car, Check, CheckCircle2, Plus, Zap } from "lucide-react"
...
export default function PaceIntake({
  ...
  const [lastRegisteredPlate, setLastRegisteredPlate] = useState<string | null>(null)
  ...
  const handleFullSubmit = async (e: React.FormEvent) => {
    ...
    setSuccessMessage(`Vehicle ${trimmedPlate} successfully registered and added to active queue!`)
    setLicensePlate("")
    setNotes("")
    setType("New")
  ...
  const isPlateValid = licensePlate.length >= 7 && isValidSwedishPlate(licensePlate)
  ...
  if (isSheet) {
    ...
    const autoService = SERVICE_TYPES.find(s => s.name === autoDefaults.serviceType)

    const presetCards = [
      {
        label: "Condition Preset:",
        value: autoDefaults.condition,
        color: CONDITION_COLORS[autoDefaults.condition],
      },
      {
        label: "Service Preset:",
        value: autoDefaults.serviceType,
        color: SERVICE_TYPE_COLORS[autoDefaults.serviceType],
        extra: autoService && `(${formatSecondsToClock(autoService.estimatedSeconds)})`,
      },
    ]

    return (
      <form onSubmit={handleQuickAddSubmit} className="space-y-6" id="pace-quick-intake-form">
        <FieldSet className="space-y-5">
          <FieldGroup className="gap-5">
            {/* License Plate Field */}
            <Field>
              <div className="flex items-center justify-between">
                <FieldLabel htmlFor="quick-license-plate-input">License Plate</FieldLabel>
                {licensePlate && (
                  <span
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pb-2 border-b border-border">
        <div>
          ...
          <p className="text-xs text-muted-foreground font-mono mt-0.5">
            Comprehensive vehicle check-in with custom detailing packages, condition scoring, and
            bay notes.
          </p>
        </div>
      </div>
            ...
            <Card className="border-border">
              <CardHeader className="border-b border-border pb-4">
                ...
                <CardDescription className="text-xs font-mono">
                  Fill in vehicle identifier and select customized package requirements.
                </CardDescription>
              </CardHeader>
                ...
                <FieldSet className="space-y-6">
                  <FieldGroup className="gap-6">
                    ...
                    <Field>
                      ...
                      />
                      <FieldDescription>
                        Standard Swedish format (e.g. ABC 123 or ABC 12A). Auto-formatted as you
                        type.
                      </FieldDescription>
                    </Field>
                    ...
                    <Field>
                      ...
                      <FieldDescription>
                        Selecting a classification sets initial recommended condition and service
                        presets.
                      </FieldDescription>
                    </Field>
              ...
              <CardFooter className="border-t border-border pt-4 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                  ...
                  <Button type="submit" disabled={loading} className="gap-2 font-mono text-xs">
                    ...
                    <span>{loading ? "Registering..." : "Register Vehicle to Fleet"}</span>
                  </Button>
                </div>
              </CardFooter>
            </Card>
                ...
                <div className="p-4 bg-muted/40 border border-border space-y-3">
                  ...
                  {notes && (
                    <div className="text-xs text-muted-foreground border-t border-border/60 pt-2 font-mono">
                      ...
                      <p className="line-clamp-3 italic text-foreground">&ldquo;{notes}&rdquo;</p>
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex justify-between py-1 border-b border-border/50">
                    ...
                    <span className="font-bold text-foreground">
                      {formatSecondsToClock(
                        SERVICE_TYPES.find(s => s.name === serviceType)?.estimatedSeconds
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-border/50">
