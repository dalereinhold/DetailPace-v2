...
import { type Stats, type TypeStats, useStats } from "@/lib/pace/stats";
import { formatDuration, formatSecondsToClock } from "@/lib/pace/vehicle";
import type { VehicleServiceType, VehicleType } from "@/lib/supabase";
...
function formatUnitLabel(value: number, unit: TimeUnit): string {
	if (unit === "hours") {
		return `${value}h`;
	}
	return `${value}m`;
...
export default function PaceStats({
	refreshTrigger,
	onRefresh,
}: PaceStatsProps) {
	...
	const actualVsTargetData = useMemo(() => {
		...
		return SERVICE_TYPES.map((s) => {
			...
			const diffPct =
				actualSec > 0 && targetSec > 0
					? Math.round((diffSec / targetSec) * 100)
					: 0;
	...
	const typeBenchmarkSummary = useMemo(() => {
		...
		return VEHICLE_TYPES.map((type) => {
			...
			const diffPct =
				avgSec > 0 && estSec > 0
					? Math.round(((avgSec - estSec) / estSec) * 100)
					: 0;
			const isFaster = diffSec < 0;
			const totalInventory = stats.totalByType[type] ?? 0;
			const completionRate =
				totalInventory > 0 ? Math.round((count / totalInventory) * 100) : 0;
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5">
				<div className="space-y-1">
					...
					<p className="text-xs text-muted-foreground font-mono">
						Combined multi-dimensional benchmarks measuring real turnaround
						times against target estimates.
					</p>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
				...
				<Card className="border-border relative overflow-hidden">
					<CardHeader className="border-b border-border/80 pb-3 flex flex-row items-center justify-between">
						...
						{overall &&
							overall.avgSeconds > 0 &&
							overall.estimatedSeconds > 0 && (
								<Badge
									variant="outline"
								...
								>
									{overall.variancePct <= 0 ? (
										...
										<span className="inline-flex items-center gap-1">
											<TrendingUp className="size-3" />+{overall.variancePct}%
											Over Target
										</span>
									)}
								</Badge>
										...
										width: `${
											overall && overall.estimatedSeconds > 0
												? Math.min(
														...
														Math.max(
															10,
															(overall.avgSeconds / overall.estimatedSeconds) *
																100,
														),
													)
												: 0
						...
						<div className="space-y-1 pt-1">
							...
							<div className="h-2 bg-muted rounded-full overflow-hidden flex">
								{VEHICLE_TYPES.map((type) => {
									...
									const widthPct =
										totalFleet > 0 ? (typeCount / totalFleet) * 100 : 0;
									return (
										<div
											key={type}
			...
			<div className="space-y-4">
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
					<div>
						<h3 className="text-base font-heading font-bold uppercase tracking-wider text-foreground flex items-center gap-2">
							...
							<span>
								Vehicle Classification Benchmarks (New vs. Used vs. Demo)
							</span>
						</h3>
						<p className="text-xs text-muted-foreground font-mono">
							Aggregated cycle times, target estimates, and pacing variance
							broken down by inventory condition.
						</p>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					{typeBenchmarkSummary.map((item) => {
						...
						const pctOfTarget =
							item.estSec > 0 && item.avgSec > 0
								? Math.round((item.avgSec / item.estSec) * 100)
								: 0;
								...
								<CardContent className="px-4 py-4 space-y-3.5">
									...
									{hasData ? (
										...
										<div className="pt-2 border-t border-border/40 text-center py-2">
											<span className="text-xs text-muted-foreground font-mono">
												No completed {item.type.toLowerCase()} vehicles logged
												yet
											</span>
										</div>
									)}
									...
									<div className="flex items-center justify-between text-[11px] font-mono text-muted-foreground pt-1">
										...
										<span className="font-semibold text-foreground">
											{item.completionRate}% throughput ({item.count}/
											{item.totalInventory})
										</span>
									</div>
								</CardContent>
				...
				<CardHeader className="border-b border-border pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
					<div>
						<CardTitle className="flex items-center gap-2 text-base font-heading font-bold uppercase">
							...
							<span>
								Comparative Duration Benchmarks by Service &amp; Classification
							</span>
						</CardTitle>
						<CardDescription className="font-mono text-xs mt-1">
							Interactive multi-bar visualizer combining timing metrics across
							vehicle types and service tiers.
						</CardDescription>
					</div>
					...
					{viewMode === "by-service" && (
						<div className="space-y-4">
							<div className="flex items-center justify-between text-xs font-mono text-muted-foreground px-1">
								<span>
									Showing duration in{" "}
									<strong className="text-foreground">{timeUnit}</strong> for
										...
										content={
											<ChartTooltipContent
												formatter={(value, name, item) => {
													...
													const count =
														item.payload[`${serviceKey} Count`] || 0;
										...
										content={
											<ChartTooltipContent
												formatter={(value, name, item) => {
													...
													const rawSec = isActual
														? item.payload.actualRaw
														: item.payload.targetRaw;
				...
				<CardHeader className="border-b border-border pb-4 flex flex-row items-center justify-between">
					<div>
						...
						<CardDescription className="font-mono text-xs mt-0.5">
							Granular cross-tabulation of average turnaround times per service
							across New, Used, and Demo classifications.
						</CardDescription>
					</div>
				</CardHeader>
					...
					<table className="w-full text-left border-collapse font-mono text-xs">
						...
						<tbody className="divide-y divide-border/60">
							{SERVICE_TYPES.map((service) => {
								...
								const hasOverall =
									(overallService?.count ?? 0) > 0 &&
									(overallService?.avgSeconds ?? 0) > 0;
								...
								return (
									...
									>
										...
										<td className="py-3.5 px-4">
											{newStats &&
											newStats.count > 0 &&
											newStats.avgSeconds > 0 ? (
												<div>
													<span className="font-bold text-foreground">
														{newStats.avgFormatted}
													</span>
													<span className="text-[10px] text-muted-foreground ml-1.5">
														({newStats.count}{" "}
														{newStats.count === 1 ? "car" : "cars"})
													</span>
												</div>
											) : (
