...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
	...
	return (
		<style
			// biome-ignore lint/security/noDangerouslySetInnerHtml: styles generation for chart variables
...
${colorConfig
	.flatMap(([key, itemConfig]) => {
		...
		const rules = [`  --color-${sanitizedKey}: ${color};`];
...
export function ChartTooltipContent({
	...
	return (
		<div
			className={cn(
				"grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl font-mono",
				className,
			)}
		>
...
function getPayloadConfigFromPayload(
	...
	const sanitizedConfigKey = configLabelKey.replace(/\s+/g, "-");
	const sanitizedKey = key.replace(/\s+/g, "-");

	return (
		config[configLabelKey] ||
		config[sanitizedConfigKey] ||
		config[key] ||
		config[sanitizedKey]
	);
}
